...
final class SnapshotAnonymizerTests: XCTestCase {
    ...
    func testSendableClosureTypeCompiles() {
        // Define a @Sendable closure and ensure it can be passed where anonymize is expected.
        let closure: @Sendable (String) -> String = { value in value + "_anon" }
        // We can't instantiate HMAccessory easily in tests, but we can at least ensure the closure type matches.
        func acceptsSendable(_ f: @Sendable (String) -> String) -> String { f("test") }
