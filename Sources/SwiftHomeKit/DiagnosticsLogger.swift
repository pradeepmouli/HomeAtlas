...
/// Represents a diagnostics event emitted by SwiftHomeKit operations.
...
public final class DiagnosticsLogger: @unchecked Sendable {
    ...
    #if canImport(OSLog)
    private let logger = Logger(subsystem: "com.pradeepmouli.SwiftHomeKit", category: "Diagnostics")
    #endif
    ...
    private func log(_ event: DiagnosticsEvent) {
        ...
        if event.outcome == .failure || event.duration >= latencyWarningThreshold {
            ...
            print("[SwiftHomeKit] \(event.operation.rawValue) :: \(event.outcome) :: \(duration)s :: \(event.metadata)")
        }
        #endif
    }
}
