...
- **Rationale**: SwiftSyntax provides first-class Swift AST construction without string concatenation, enabling reliable generation across Swift 6 toolchains. Running the generator through an SPM command plugin keeps the workflow co-located with package consumers (`swift package plugin generate-homekit`) and supports CI automation.
...
- **Decision**: Ship the library as a single SPM package with one library target (`SwiftHomeKit`), one executable target (`homekit-schema-gen`), and one command plugin target (`SwiftHomeKitPlugin`). Publish semantic versions alongside constitution updates.
- **Rationale**: SPM is the mandated distribution mechanism (FR-007). Packaging generator and plugin alongside the library keeps tooling discoverable, allows `swift package plugin generate-homekit` to refresh sources, and avoids parallel distribution channels.
